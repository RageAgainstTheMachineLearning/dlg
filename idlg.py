...
class iDLG:
    def __init__(self, model, loss, grad, input_size, output_size, batch_size, device, step_size=0.1):
        """
        A simple iDLG optimizer with a fixed step size.
    ...
    def optimize(self, gt, *, max_iter=100, print_it=10, plot_it=50, plot_fn=None):
        """
        ...
        - plot_it: Plot the target image every `plot_it` iterations.
        ...
        optimizer = torch.optim.LBFGS([self.target_data, ],  lr=1,
                                      tolerance_grad=1e-7, tolerance_change=1e-9, max_iter=100,
                                      line_search_fn="strong_wolfe")

        # iDLG trick.
        label_pred = torch.argmin(
            torch.sum(gt[-2], dim=-1), dim=-1).detach().reshape((1,)).requires_grad_(False)

        def closure():
            optimizer.zero_grad()
            g = self._grad(label_pred)
...
if __name__ == '__main__':
    ...
    print("Optimizing...")
    dlg = iDLG(model, loss_fn, grad_fn, input_size,
